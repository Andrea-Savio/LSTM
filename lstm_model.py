...
class LSTModel(object):
	...
	def __init__(self, hidden_dim: torch.Size , window_size: int, num_layers: int, use_checkpoint: bool):
		...
		self.lstm1 = nn.LSTM(hidden_dim, output_dim, num_layers=num_layers)
	...
	# Run training and save resulting model

	def run(self, window_size, train_data, criterion, optimizer):

		# Load data

		X_train = train_data[:(len(train_data)-window_size),:,:]
		...
		for person in range(len(train_data[0][0])):
			for i in range(len(train_data)/window_size):
				
				# Get a batch of data

				inputs = X_train[(i*window_size):(i*window_size + window_size),:,person]
				targets = Y_train[(i*window_size):(i*window_size + window_size),:,person]
		...
		save_model(self, 'models/model.pth')
	...
	def load_model(model, path):
		#model = MultiLayerLSTM(input_dim, hidden_dim, num_layers, output_dim)
...
if __name__ == "__main__":
	...
	# Prepare the training data and set time window (an increase of 7.5 corresponds to 1 second)
