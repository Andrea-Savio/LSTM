...
class LSTM_Trainer(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers, seq_length):
        ...
        self.linear_context = nn.Linear(8,256)
...
if __name__ == "__main__":
    ...
    x = torch.randn(1 ,35, 10)
    y = model(x[:,:,0:2], x[:,:,2:10])
    #make_dot(y, params=dict(list(model.named_parameters()))).render("rnn_torchviz_2", format="png")
    #model.load_state_dict(torch.load("models/model_complete/20_epochs.pt"))
    ...
    for epoch in range(num_epochs):
        ...
        for file in train_files:
            ...
            for X_train, Y_train in train_dataloader:
                ...
                for i in range(num_subsequences):
                    ...
                    subseq_cont = X_train[:,i:i+seq_length,3:].float()
                   ...
                   # print(subseq_xyz.shape)
                    ...
                    preds_xyz, preds_cont = model(subseq_xyz, subseq_cont)
                    preds_cont = preds_cont[:,-1,:]
                    #print(preds_xyz)
                    #print(preds_cont)
                    ...
                    loss = torch.sqrt(loss + eps) #+ l2_lambda * l2_norm
        ...
        print("Loss: " + str(loss))
        if epoch%10 == 0:
            torch.save(model.state_dict(), "models/model_complete/3lstm_2d_" + "checkpoint_" + str(epoch) + ".pt") 
    ...
    for file in test_files:
        ...
        for X_test, Y_test in test_dataloader:
            ...
            for j in range(num_subsequences):
                ...
                subseq_cont = X_test[:,j:j+seq_length,3:].float()
                #subseq_xyz = torch.tensor(subseq_xyz, dtype=torch.float32)     
                subseq_xyz = subseq_xyz.to(device)
                subseq_cont = subseq_cont.to(device)
    ...
    torch.save(model.state_dict(), "models/model_complete/3lstm_2d_complete.pt") 
