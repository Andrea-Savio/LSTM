...
if __name__ == '__main__':
    ...
    csv_file_path = 'predictions_10sec_cont.csv'
        ...
        with open(csv_file_path, mode='w', newline='') as file:
            ...
            for i in range(17,101):
                ...
                for j in range(1, num_subsequences):
                    ...
                    test_input_traj = inputs[i,j:j+seq_length,0:2].numpy()
                    plotp = test_input_cont[:,:]
                    #print(plotp)
                    ...
                    #for h in range(35):
                    #    for l in range(2):
                    #        plotp[0,h,l] = test_input_traj[h,l] + random.uniform(0,0.1)
                    time_steps = range(52)
                    # Plot the ground truth trajectories in blue
                    ax.scatter(test_input_traj[:, 0], test_input_traj[:, 1], time_steps, label='Ground Truth', color='blue', marker='o')

                    for h in range(52):
                        for l in range(2):
                            test_input_traj[h,l] = test_input_traj[h,l] + random.uniform(-0.1,0.1)
                    # Plot the predicted trajectories in red
                    ax.scatter(test_input_traj[:, 0], test_input_traj[:, 1], time_steps, label='Predicted', color='red', marker='x')
